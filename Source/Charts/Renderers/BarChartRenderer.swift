...
//
//            context.fill(barRect)
            drawBar(context: context, barRect: barRect)
            if drawBorder
            {
                context.saveGState()
    ...
    {
        let largeNumber = Double(value) ?? 0
        let numberFormatter = NumberFormatter()
        numberFormatter.numberStyle = .decimal
        let formattedNumber = numberFormatter.string(from: NSNumber(value:largeNumber)) ?? value
        
        ChartUtils.drawText(context: context, text: formattedNumber, point: CGPoint(x: xPos, y: yPos), align: align, attributes: [NSAttributedString.Key.font: font, NSAttributedString.Key.foregroundColor: color])
    }
    ...
    open func drawBar(context: CGContext, barRect: CGRect)
    {
        context.saveGState()
        defer { context.restoreGState() }
           
        drawGradient(context: context, barRect: barRect)
    }
       
    open func drawGradient(context: CGContext, barRect: CGRect)
    {
        let gradientColors: [NSUIColor] = [UIColor(red: 207 / 255.0, green: 109 / 255.0, blue: 246 / 255.0, alpha: 1), UIColor(red: 249 / 255.0, green: 133 / 255.0, blue: 98 / 255.0, alpha: 1)]
